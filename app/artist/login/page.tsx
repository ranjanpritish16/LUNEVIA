...
// ✅ Inner component with all the form logic
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "signin") {
      ...
      if (error) {
        ...
        // Check the user's role — artists only
        ...
        if (profile?.role !== "artist") {
          setMessage({
            type: "error",
            text: "This account is registered as a customer. Please use the bride login instead.",
            link: { href: "/login/customer", label: "Go to bride login" },
          });
      ...
      if (error) {
        ...
        // Upsert into profiles with role: 'artist'
        if (data.user) {
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <h2 className="mb-1 text-center font-cormorant text-3xl font-semibold text-primary">
                {mode === "signin" ? "Welcome back, Artist" : mode === "signup" ? "Join LUNÉVIA" : "Reset password"}
              </h2>
              <p className="mb-7 text-center font-dm-sans text-sm text-charcoal/60">
                {mode === "signin"
                  ? "Sign in to manage your salon."
                  : mode === "signup"
                    ? "Showcase your work to Delhi's brides."
                    : "We'll send a reset link to your inbox."}
              </p>

              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label htmlFor="artist-email" className="mb-1.5 block font-dm-sans text-xs font-medium uppercase tracking-widest text-charcoal/50">
                    Email Address
                  </label>
                ...
                {mode !== "forgot" && (
                  <div>
                    <div className="mb-1.5 flex items-center justify-between">
                      <label htmlFor="artist-password" className="block font-dm-sans text-xs font-medium uppercase tracking-widest text-charcoal/50">
                        Password
                      </label>
                      {mode === "signin" && (
              ...
              <p className="mt-6 text-center font-dm-sans text-sm text-charcoal/60">
                {mode === "signin" ? (
                  <>New artist?{" "}<button onClick={() => switchMode("signup")} className="font-medium text-gold transition-colors hover:text-gold/70">Apply to join LUNÉVIA</button></>
                ) : mode === "signup" ? (
                  <>Already registered?{" "}<button onClick={() => switchMode("signin")} className="font-medium text-gold transition-colors hover:text-gold/70">Sign in</button></>
                ) : (
...
// ✅ Default export wraps ArtistLoginContent in Suspense
